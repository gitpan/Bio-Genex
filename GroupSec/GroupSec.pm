...
# Bio::Genex::GroupSec
#
# created on Mon Jan 15 11:06:31 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=GroupSec --support=GroupLink
#
# cvs id: $Id: GroupSec.pm,v 1.16.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
;
 $FKEYS = {
          'grouplink_fk' => bless( {
                                     'table_name' => 'GroupLink',
                                     'fkey_type' => 'ONE_TO_MANY_LINK',
                                     'fkey_name' => 'grouplink_fk',
                                     'pkey_name' => 'gs_fk'
                                   }, 'Bio::Genex::Fkey' ),
          'grouplink_obj' => bless( {
                                      'fkey_type' => 'ONE_TO_MANY_LINK_OO',
                                      'table_name' => 'GroupLink',
                                      'fkey_name' => 'grouplink_obj',
                                      'pkey_name' => 'gs_fk'
                                    }, 'Bio::Genex::Fkey' )
        }
;
  ...
  $COLUMN2NAME  = {
          ...
          'gs_pk' => 'Accession Number'
        }
;
  $NAME2COLUMN  = {
          'Group Name' => 'group_name',
          'Accession Number' => 'gs_pk'
        }
;
  $FKEY_OBJ2RAW = {
          'grouplink_obj' => 'grouplink_fk'
        }
;
}


attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['gs_pk', 'group_name', 'grouplink_obj', 'grouplink_fk']);

sub table_name {return 'GroupSec';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'GroupSec',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'gs_pk=' . $dbh->quote($self->gs_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::GroupSec::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::GroupSec::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::GroupSec::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::GroupSec::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::GroupSec::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::GroupSec::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::GroupSec::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::GroupSec::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['gs_pk', 'group_name'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::GroupSec::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::GroupSec::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "GroupSec: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::GroupSec - Methods for processing data from the GeneX DB
 table: GroupSec
...
=head1 SYNOPSIS
  ...
  my $GroupSec = Bio::Genex::GroupSec->new(id=>47);

  # retrieve data from the DB for all columns
  $GroupSec->fetch();

  # creating an instance, without pre-fetching all columns
  my $GroupSec = new Bio::Genex::GroupSec(id=>47);

  # creating an instance with pre-fetched data
  my $GroupSec = new Bio::Genex::GroupSec(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::GroupSec->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::GroupSec->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $GroupSec->id();

  # or specifically
  my $gs_pk_val = $GroupSec->gs_pk();

  # retreving other DB column attributes
  my $group_name_val = $GroupSec->group_name();
  $GroupSec->group_name($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::GroupSec is
GroupSec).
...
Bio::Genex::GroupSec can access: I<raw> foreign key attributes,
...
Class Bio::Genex::GroupSec defines the following utility variables for assisting
programmers to access the GroupSec table.

=over 4

=item $Bio::Genex::GroupSec::LIMIT
...
=item $Bio::Genex::GroupSec::USE_CACHE
...
instance of the class via the Bio::Genex::GroupSec->methodname() syntax.
...
linking table class. For Bio::Genex::GroupSec it returns 0, since it is
...
table classes, and for Bio::Genex::GroupSec it returns the value 'gs_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::GroupSec it returns 'GroupSec';
...
C<insert_db()>.
  ...
  my GroupSec = Bio::Genex::GroupSec->new();
  GroupSec->group_name('some_value');
  GroupSec->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my GroupSec = Bio::Genex::GroupSec->new(id=>43);
  GroupSec->group_name('some_value');
  GroupSec->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('gs_pk' for module Bio::Genex::GroupSec).
...
This method is used to retrieve multiple instances of class Bio::Genex::GroupSec
...
list as a primary key for the GroupSec table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::GroupSec::initialize()>. Objects will be
...
instance of class Bio::Genex::GroupSec.
...
(and for class Bio::Genex::GroupSec it is synonomous with the
C<gs_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item gs_pk()

This is the primary key attribute for Bio::Genex::GroupSec. It has no setter method. 
...
Methods for the group_name attribute.
...
on Mon Jan 15 11:06:31 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=GroupSec --support=GroupLink
