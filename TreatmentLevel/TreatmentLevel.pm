...
#
# created on Mon Jan 15 11:06:29 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=TreatmentLevel --support=TL_FactorValues --support=Treatment_AMs
#
# cvs id: $Id: TreatmentLevel.pm,v 1.9.2.1 2001/01/15 18:52:02 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'tl_pk' => 'Accession Number',
          'name' => 'Name',
          'ratio_am_fk' => 'Ratio Array Measurement'
        }
;
  $NAME2COLUMN  = {
          'Accession Number' => 'tl_pk',
          'Name' => 'name',
          'Ratio Array Measurement' => 'ratio_am_fk',
          'Experiment Set' => 'es_fk'
        }
;
  $FKEY_OBJ2RAW = {
          'ratio_am_obj' => 'ratio_am_fk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['tl_pk', 'es_fk', 'ratio_am_fk', 'name', 'ratio_am_obj', 'treatment_ams_obj', 'treatment_ams_fk', 'tl_factorvalues_obj', 'tl_factorvalues_fk', 'es_obj']);

sub table_name {return 'TreatmentLevel';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'TreatmentLevel',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'tl_pk=' . $dbh->quote($self->tl_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::TreatmentLevel::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::TreatmentLevel::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::TreatmentLevel::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::TreatmentLevel::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::TreatmentLevel::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::TreatmentLevel::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::TreatmentLevel::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::TreatmentLevel::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['tl_pk', 'es_fk', 'ratio_am_fk', 'name'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::TreatmentLevel::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::TreatmentLevel::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "TreatmentLevel: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
on Mon Jan 15 11:06:29 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=TreatmentLevel --support=TL_FactorValues --support=Treatment_AMs
