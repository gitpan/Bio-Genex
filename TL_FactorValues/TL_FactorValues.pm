...
#
# created on Mon Jan 15 11:06:37 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=TL_FactorValues
#
# cvs id: $Id: TL_FactorValues.pm,v 1.9.2.1 2001/01/15 18:52:02 jes Exp $ 
#
##############################
...
;
  $TABLE2PKEY = {
          'ExperimentFactors' => 'tl_fk',
          'TreatmentLevel' => 'ef_fk'
        }
;
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['ef_fk', 'tl_fk', 'value', 'reference_value', 'ef_obj', 'tl_obj']);

sub table_name {return 'TL_FactorValues';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::TL_FactorValues::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::TL_FactorValues::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::TL_FactorValues::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::TL_FactorValues::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::TL_FactorValues::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::TL_FactorValues::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::TL_FactorValues::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::TL_FactorValues::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::TL_FactorValues::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::TL_FactorValues::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::TL_FactorValues::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::TL_FactorValues::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['ef_fk', 'tl_fk', 'value', 'reference_value'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::TL_FactorValues::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::TL_FactorValues::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "TL_FactorValues: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::TL_FactorValues - Methods for processing data from the GeneX DB
linking table: TL_FactorValues

=head1 SYNOPSIS

  use Bio::Genex::TL_FactorValues;

  # instantiating a linking table instance
  my $TL_FactorValues = Bio::Genex::TL_FactorValues->new(id=>47,pkey_link=>'ef_fk');
  # or 
  my $TL_FactorValues = Bio::Genex::TL_FactorValues->new(id=>47,pkey_link=>'tl_fk');

  # retrieve data from the DB for all columns
  $TL_FactorValues->fetch();

  # creating an instance, without pre-fetching all columns
  my $TL_FactorValues = new Bio::Genex::TL_FactorValues(id=>47,
                                              'pkey_link'=>'ef_fk');
  ...
  my @objects = Bio::Genex::TL_FactorValues->get_objects('pkey_link'=>'ef_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::TL_FactorValues->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $TL_FactorValues->id();

  # retrieving other DB column attributes
  my $ef_fk_val = $TL_FactorValues->ef_fk();
  $TL_FactorValues->ef_fk($value);

  my $tl_fk_val = $TL_FactorValues->tl_fk();
  $TL_FactorValues->tl_fk($value);

  my $value_val = $TL_FactorValues->value();
  $TL_FactorValues->value($value);

  my $reference_value_val = $TL_FactorValues->reference_value();
  $TL_FactorValues->reference_value($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::TL_FactorValues is
TL_FactorValues).


B<NOTE>: Class Bio::Genex::TL_FactorValues is linking table class. This means
...
value from the database. In Bio::Genex::TL_FactorValues, those two columns are
'ef_fk' and 'ef_fk'. When an instance of
Bio::Genex::TL_FactorValues is instantiated, either via C<new()> or
...
Bio::Genex::TL_FactorValues can access: I<raw> foreign key attributes,
...
Class Bio::Genex::TL_FactorValues defines the following utility variables for assisting
programmers to access the TL_FactorValues table.

=over 4

=item $Bio::Genex::TL_FactorValues::LIMIT
...
=item $Bio::Genex::TL_FactorValues::USE_CACHE
...
For example:
  ...
  my $TL_FactorValues = Bio::Genex::TL_FactorValues->new(id=>47);
  my $val = $TL_FactorValues->ef_fk();
...
instance of the class via the Bio::Genex::TL_FactorValues->methodname() syntax.
...
DB. For class Bio::Genex::TL_FactorValues, 'C<pkey_link>' can be either 'C<ef_fk>' or 'C<tl_fk>'.
...
Bio::Genex::TL_FactorValues if a method in class C<ExperimentFactors> invokes its
...
linking table class. For Bio::Genex::TL_FactorValues it returns 1, since it I<is>
...
Bio::Genex::TL_FactorValues it returns 'TL_FactorValues';
...
Bio::Genex::TL_FactorValues simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the TL_FactorValues table and returns a single
...
a warning from C<Bio::Genex::TL_FactorValues::initialize()>. Objects will be
...
instance of class Bio::Genex::TL_FactorValues.
...
instance. Class Bio::Genex::TL_FactorValues is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<ef_fk>' and 'C<tl_fk>'). When an instance of
class Bio::Genex::TL_FactorValues is created using either C<new()> or
...
=item $value = reference_value();

=item reference_value($value);

Methods for the reference_value attribute.
...
on Mon Jan 15 11:06:37 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=TL_FactorValues
