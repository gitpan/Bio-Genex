...
# Bio::Genex::Protocol
#
# created on Mon Jan 15 11:06:31 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=Protocol --support=SampleProtocols
#
# cvs id: $Id: Protocol.pm,v 1.13.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'title' => 'Title',
          'type' => 'Type',
          'prt_pk' => 'Accession Number'
        }
;
  $NAME2COLUMN  = {
          'Text' => 'text',
          'Title' => 'title',
          'Accession Number' => 'prt_pk',
          'Type' => 'type'
        }
;
  $FKEY_OBJ2RAW = {
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['prt_pk', 'title', 'type', 'text', 'sampleprotocols_obj', 'sampleprotocols_fk']);

sub table_name {return 'Protocol';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'Protocol',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'prt_pk=' . $dbh->quote($self->prt_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::Protocol::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::Protocol::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::Protocol::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::Protocol::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::Protocol::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::Protocol::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::Protocol::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::Protocol::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['prt_pk', 'title', 'type', 'text'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::Protocol::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::Protocol::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "Protocol: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::Protocol - Methods for processing data from the GeneX DB
 table: Protocol
...
=head1 SYNOPSIS
  ...
  my $Protocol = Bio::Genex::Protocol->new(id=>47);

  # retrieve data from the DB for all columns
  $Protocol->fetch();

  # creating an instance, without pre-fetching all columns
  my $Protocol = new Bio::Genex::Protocol(id=>47);

  # creating an instance with pre-fetched data
  my $Protocol = new Bio::Genex::Protocol(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::Protocol->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::Protocol->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $Protocol->id();

  # or specifically
  my $prt_pk_val = $Protocol->prt_pk();
...
of the same name (I<i.e.> the corresponding table for Bio::Genex::Protocol is
Protocol).
...
Bio::Genex::Protocol can access: I<raw> foreign key attributes,
...
Class Bio::Genex::Protocol defines the following utility variables for assisting
programmers to access the Protocol table.

=over 4

=item $Bio::Genex::Protocol::LIMIT
...
=item $Bio::Genex::Protocol::USE_CACHE
...
instance of the class via the Bio::Genex::Protocol->methodname() syntax.
...
linking table class. For Bio::Genex::Protocol it returns 0, since it is
...
table classes, and for Bio::Genex::Protocol it returns the value 'prt_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::Protocol it returns 'Protocol';
...
C<insert_db()>.
  ...
  my Protocol = Bio::Genex::Protocol->new();
  Protocol->title('some_value');
  Protocol->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my Protocol = Bio::Genex::Protocol->new(id=>43);
  Protocol->title('some_value');
  Protocol->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('prt_pk' for module Bio::Genex::Protocol).
...
This method is used to retrieve multiple instances of class Bio::Genex::Protocol
...
list as a primary key for the Protocol table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::Protocol::initialize()>. Objects will be
...
instance of class Bio::Genex::Protocol.
...
(and for class Bio::Genex::Protocol it is synonomous with the
C<prt_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item prt_pk()

This is the primary key attribute for Bio::Genex::Protocol. It has no setter method. 
...
Methods for the text attribute.
...
on Mon Jan 15 11:06:31 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=Protocol --support=SampleProtocols
