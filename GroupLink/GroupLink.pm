...
# Bio::Genex::GroupLink
#
# created on Mon Jan 15 11:06:37 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=GroupLink
#
# cvs id: $Id: GroupLink.pm,v 1.9.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['us_fk', 'gs_fk', 'us_obj', 'gs_obj']);

sub table_name {return 'GroupLink';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::GroupLink::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::GroupLink::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::GroupLink::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::GroupLink::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::GroupLink::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::GroupLink::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::GroupLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::GroupLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::GroupLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::GroupLink::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::GroupLink::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::GroupLink::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::GroupLink::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::GroupLink::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "GroupLink: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::GroupLink - Methods for processing data from the GeneX DB
linking table: GroupLink
...
=head1 SYNOPSIS
  ...
  my $GroupLink = Bio::Genex::GroupLink->new(id=>47,pkey_link=>'us_fk');
  # or 
  my $GroupLink = Bio::Genex::GroupLink->new(id=>47,pkey_link=>'gs_fk');

  # retrieve data from the DB for all columns
  $GroupLink->fetch();

  # creating an instance, without pre-fetching all columns
  my $GroupLink = new Bio::Genex::GroupLink(id=>47,
                                              'pkey_link'=>'us_fk');
  ...
  my @objects = Bio::Genex::GroupLink->get_objects('pkey_link'=>'us_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::GroupLink->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $GroupLink->id();

  # retrieving other DB column attributes
  my $us_fk_val = $GroupLink->us_fk();
  $GroupLink->us_fk($value);

  my $gs_fk_val = $GroupLink->gs_fk();
  $GroupLink->gs_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::GroupLink is
GroupLink).


B<NOTE>: Class Bio::Genex::GroupLink is linking table class. This means
...
value from the database. In Bio::Genex::GroupLink, those two columns are
'us_fk' and 'us_fk'. When an instance of
Bio::Genex::GroupLink is instantiated, either via C<new()> or
...
Bio::Genex::GroupLink can access: I<raw> foreign key attributes,
...
Class Bio::Genex::GroupLink defines the following utility variables for assisting
programmers to access the GroupLink table.

=over 4

=item $Bio::Genex::GroupLink::LIMIT
...
=item $Bio::Genex::GroupLink::USE_CACHE
...
For example:
  ...
  my $GroupLink = Bio::Genex::GroupLink->new(id=>47);
  my $val = $GroupLink->us_fk();
...
instance of the class via the Bio::Genex::GroupLink->methodname() syntax.
...
DB. For class Bio::Genex::GroupLink, 'C<pkey_link>' can be either 'C<us_fk>' or 'C<gs_fk>'.
...
Bio::Genex::GroupLink if a method in class C<UserSec> invokes its
foreign key accessor function, the column 'gs_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::GroupLink it returns 1, since it I<is>
...
Bio::Genex::GroupLink it returns 'GroupLink';
...
=item get_objects({pkey_link=>'us_fk'}, @id_list)
...
Bio::Genex::GroupLink simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the GroupLink table and returns a single
...
a warning from C<Bio::Genex::GroupLink::initialize()>. Objects will be
...
instance of class Bio::Genex::GroupLink.
...
instance. Class Bio::Genex::GroupLink is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<us_fk>' and 'C<gs_fk>'). When an instance of
class Bio::Genex::GroupLink is created using either C<new()> or
...
Methods for the us_fk attribute. This attribute is the first
...
Methods for the gs_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
on Mon Jan 15 11:06:37 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=GroupLink
