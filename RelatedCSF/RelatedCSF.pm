...
#
# created on Mon Jan 15 11:06:36 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=RelatedCSF
#
# cvs id: $Id: RelatedCSF.pm,v 1.9.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
package Bio::Genex::RelatedCSF;
...
;
 $FKEYS = {
          'csf1_csf_obj' => bless( {
                                     ...
                                     'fkey_name' => 'csf1_csf_obj',
                                     'pkey_name' => 'csf_pk'
                                   }, 'Bio::Genex::Fkey' ),
          'csf2_csf_obj' => bless( {
                                     'fkey_type' => 'LINKING_TABLE_OO',
                                     'table_name' => 'CanonicalSequenceFeature',
                                     'fkey_name' => 'csf2_csf_obj',
                                     'pkey_name' => 'csf_pk'
                                   }, 'Bio::Genex::Fkey' )
        }
;
  $TABLE2PKEY = {
          'CanonicalSequenceFeature' => 'csf1_csf_fk'
        }
;

  $COLUMN2NAME  = {
          'csf2_csf_fk' => 'Canonical Sequence Feature Two',
          'csf1_csf_fk' => 'Canonical Sequence Feature One'
        }
;
  $NAME2COLUMN  = {
          'Canonical Sequence Feature One' => 'csf1_csf_fk',
          'Canonical Sequence Feature Two' => 'csf2_csf_fk'
        }
;
  $FKEY_OBJ2RAW = {
          'csf1_csf_obj' => 'csf1_csf_fk',
          'csf2_csf_obj' => 'csf2_csf_fk'
        }
;
}


attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['csf1_csf_fk', 'csf2_csf_fk', 'csf1_csf_obj', 'csf2_csf_obj']);

sub table_name {return 'RelatedCSF';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::RelatedCSF::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::RelatedCSF::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::RelatedCSF::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::RelatedCSF::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::RelatedCSF::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::RelatedCSF::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::RelatedCSF::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::RelatedCSF::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::RelatedCSF::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::RelatedCSF::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::RelatedCSF::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::RelatedCSF::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['csf1_csf_fk', 'csf2_csf_fk'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::RelatedCSF::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::RelatedCSF::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "RelatedCSF: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::RelatedCSF - Methods for processing data from the GeneX DB
linking table: RelatedCSF

=head1 SYNOPSIS

  use Bio::Genex::RelatedCSF;

  # instantiating a linking table instance
  my $RelatedCSF = Bio::Genex::RelatedCSF->new(id=>47,pkey_link=>'csf1_csf_fk');
  # or 
  my $RelatedCSF = Bio::Genex::RelatedCSF->new(id=>47,pkey_link=>'csf2_csf_fk');
  ...
  my @objects = Bio::Genex::RelatedCSF->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $RelatedCSF->id();

  # retrieving other DB column attributes
  my $csf1_csf_fk_val = $RelatedCSF->csf1_csf_fk();
  $RelatedCSF->csf1_csf_fk($value);

  my $csf2_csf_fk_val = $RelatedCSF->csf2_csf_fk();
  $RelatedCSF->csf2_csf_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::RelatedCSF is
RelatedCSF).


B<NOTE>: Class Bio::Genex::RelatedCSF is linking table class. This means
...
Bio::Genex::RelatedCSF is instantiated, either via C<new()> or
...
Most applications will first create an instance of Bio::Genex::RelatedCSF
...
Bio::Genex::RelatedCSF can access: I<raw> foreign key attributes,
...
Class Bio::Genex::RelatedCSF defines the following utility variables for assisting
programmers to access the RelatedCSF table.

=over 4

=item $Bio::Genex::RelatedCSF::LIMIT
...
=item $Bio::Genex::RelatedCSF::USE_CACHE
...
instance of the class via the Bio::Genex::RelatedCSF->methodname() syntax.
...
DB. For class Bio::Genex::RelatedCSF, 'C<pkey_link>' can be either 'C<csf1_csf_fk>' or 'C<csf2_csf_fk>'.
...
Bio::Genex::RelatedCSF if a method in class C<CanonicalSequenceFeature> invokes its
...
linking table class. For Bio::Genex::RelatedCSF it returns 1, since it I<is>
...
=item get_objects({pkey_link=>'csf1_csf_fk'}, @id_list)
...
a warning from C<Bio::Genex::RelatedCSF::initialize()>. Objects will be
...
instance of class Bio::Genex::RelatedCSF.
...
('C<csf1_csf_fk>' and 'C<csf2_csf_fk>'). When an instance of
class Bio::Genex::RelatedCSF is created using either C<new()> or
...
on Mon Jan 15 11:06:36 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=RelatedCSF
