...
#
# created on Mon Jan 15 11:06:35 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=AM_SuspectSpots
#
# cvs id: $Id: AM_SuspectSpots.pm,v 1.9.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
...
;
  $TABLE2PKEY = {
          'AM_Spots' => 'am_fk',
          'ArrayMeasurement' => 'ams_fk'
        }
;

  $COLUMN2NAME  = {
          'ams_fk' => 'Array Measurement Spot',
          'am_fk' => 'Array Measurement'
        }
;
  $NAME2COLUMN  = {
          'Array Measurement Spot' => 'ams_fk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['am_fk', 'ams_fk', 'am_obj', 'ams_obj']);

sub table_name {return 'AM_SuspectSpots';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::AM_SuspectSpots::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::AM_SuspectSpots::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::AM_SuspectSpots::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::AM_SuspectSpots::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::AM_SuspectSpots::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::AM_SuspectSpots::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::AM_SuspectSpots::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::AM_SuspectSpots::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::AM_SuspectSpots::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::AM_SuspectSpots::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::AM_SuspectSpots::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::AM_SuspectSpots::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['am_fk', 'ams_fk'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::AM_SuspectSpots::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::AM_SuspectSpots::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "AM_SuspectSpots: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::AM_SuspectSpots - Methods for processing data from the GeneX DB
linking table: AM_SuspectSpots
...
=head1 SYNOPSIS
  ...
  my $AM_SuspectSpots = Bio::Genex::AM_SuspectSpots->new(id=>47,pkey_link=>'am_fk');
  # or 
  my $AM_SuspectSpots = Bio::Genex::AM_SuspectSpots->new(id=>47,pkey_link=>'ams_fk');

  # retrieve data from the DB for all columns
  $AM_SuspectSpots->fetch();

  # creating an instance, without pre-fetching all columns
  my $AM_SuspectSpots = new Bio::Genex::AM_SuspectSpots(id=>47,
                                              'pkey_link'=>'am_fk');
  ...
  my @objects = Bio::Genex::AM_SuspectSpots->get_objects('pkey_link'=>'am_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::AM_SuspectSpots->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $AM_SuspectSpots->id();

  # retrieving other DB column attributes
  my $am_fk_val = $AM_SuspectSpots->am_fk();
  $AM_SuspectSpots->am_fk($value);

  my $ams_fk_val = $AM_SuspectSpots->ams_fk();
  $AM_SuspectSpots->ams_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::AM_SuspectSpots is
AM_SuspectSpots).


B<NOTE>: Class Bio::Genex::AM_SuspectSpots is linking table class. This means
...
value from the database. In Bio::Genex::AM_SuspectSpots, those two columns are
'am_fk' and 'am_fk'. When an instance of
Bio::Genex::AM_SuspectSpots is instantiated, either via C<new()> or
...
Bio::Genex::AM_SuspectSpots can access: I<raw> foreign key attributes,
...
Class Bio::Genex::AM_SuspectSpots defines the following utility variables for assisting
programmers to access the AM_SuspectSpots table.

=over 4

=item $Bio::Genex::AM_SuspectSpots::LIMIT
...
=item $Bio::Genex::AM_SuspectSpots::USE_CACHE
...
For example:
  ...
  my $AM_SuspectSpots = Bio::Genex::AM_SuspectSpots->new(id=>47);
  my $val = $AM_SuspectSpots->am_fk();
...
instance of the class via the Bio::Genex::AM_SuspectSpots->methodname() syntax.
...
DB. For class Bio::Genex::AM_SuspectSpots, 'C<pkey_link>' can be either 'C<am_fk>' or 'C<ams_fk>'.
...
Bio::Genex::AM_SuspectSpots if a method in class C<ArrayMeasurement> invokes its
foreign key accessor function, the column 'ams_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::AM_SuspectSpots it returns 1, since it I<is>
...
Bio::Genex::AM_SuspectSpots it returns 'AM_SuspectSpots';
...
Bio::Genex::AM_SuspectSpots simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the AM_SuspectSpots table and returns a single
...
a warning from C<Bio::Genex::AM_SuspectSpots::initialize()>. Objects will be
...
instance of class Bio::Genex::AM_SuspectSpots.
...
instance. Class Bio::Genex::AM_SuspectSpots is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<am_fk>' and 'C<ams_fk>'). When an instance of
class Bio::Genex::AM_SuspectSpots is created using either C<new()> or
...
Methods for the ams_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
on Mon Jan 15 11:06:35 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=AM_SuspectSpots
