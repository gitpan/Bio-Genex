...
# Bio::Genex::Spotter
#
# created on Mon Jan 15 11:06:34 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=Spotter
#
# cvs id: $Id: Spotter.pm,v 1.13.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          'sptr_pk' => 'Accession Number',
          'model_description' => 'Model Description',
          'model_serial_num' => 'Model Serial Number',
          'con_fk' => 'Contact'
        }
;
  $NAME2COLUMN  = {
          'Contact' => 'con_fk',
          'Accession Number' => 'sptr_pk',
          'Model Description' => 'model_description',
          'Model Serial Number' => 'model_serial_num'
        }
;
  $FKEY_OBJ2RAW = {
          'con_obj' => 'con_fk'
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['sptr_pk', 'con_fk', 'model_serial_num', 'model_description', 'con_obj']);

sub table_name {return 'Spotter';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'Spotter',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'sptr_pk=' . $dbh->quote($self->sptr_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::Spotter::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::Spotter::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::Spotter::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::Spotter::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::Spotter::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::Spotter::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::Spotter::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::Spotter::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['sptr_pk', 'con_fk', 'model_serial_num', 'model_description'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::Spotter::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::Spotter::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "Spotter: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::Spotter - Methods for processing data from the GeneX DB
 table: Spotter
...
=head1 SYNOPSIS
  ...
  my $Spotter = Bio::Genex::Spotter->new(id=>47);

  # retrieve data from the DB for all columns
  $Spotter->fetch();

  # creating an instance, without pre-fetching all columns
  my $Spotter = new Bio::Genex::Spotter(id=>47);

  # creating an instance with pre-fetched data
  my $Spotter = new Bio::Genex::Spotter(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::Spotter->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::Spotter->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $Spotter->id();

  # or specifically
  my $sptr_pk_val = $Spotter->sptr_pk();

  # retreving other DB column attributes
  my $con_fk_val = $Spotter->con_fk();
  $Spotter->con_fk($value);

  my $model_serial_num_val = $Spotter->model_serial_num();
  $Spotter->model_serial_num($value);

  my $model_description_val = $Spotter->model_description();
  $Spotter->model_description($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::Spotter is
Spotter).
...
Bio::Genex::Spotter can access: I<raw> foreign key attributes,
...
Class Bio::Genex::Spotter defines the following utility variables for assisting
programmers to access the Spotter table.

=over 4

=item $Bio::Genex::Spotter::LIMIT
...
=item $Bio::Genex::Spotter::USE_CACHE
...
instance of the class via the Bio::Genex::Spotter->methodname() syntax.
...
linking table class. For Bio::Genex::Spotter it returns 0, since it is
...
table classes, and for Bio::Genex::Spotter it returns the value 'sptr_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::Spotter it returns 'Spotter';
...
C<insert_db()>.
  ...
  my Spotter = Bio::Genex::Spotter->new();
  Spotter->con_fk('some_value');
  Spotter->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my Spotter = Bio::Genex::Spotter->new(id=>43);
  Spotter->con_fk('some_value');
  Spotter->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('sptr_pk' for module Bio::Genex::Spotter).
...
This method is used to retrieve multiple instances of class Bio::Genex::Spotter
...
list as a primary key for the Spotter table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::Spotter::initialize()>. Objects will be
...
instance of class Bio::Genex::Spotter.
...
(and for class Bio::Genex::Spotter it is synonomous with the
C<sptr_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item sptr_pk()

This is the primary key attribute for Bio::Genex::Spotter. It has no setter method. 
...
=item $value = model_serial_num();

=item model_serial_num($value);

Methods for the model_serial_num attribute.


=item $value = model_description();

=item model_description($value);

Methods for the model_description attribute.
...
on Mon Jan 15 11:06:34 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=Spotter
