...
#
# created on Mon Jan 15 11:06:38 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=UserCanonicalLink
#
# cvs id: $Id: UserCanonicalLink.pm,v 1.9.2.1 2001/01/15 18:52:02 jes Exp $ 
#
##############################
package Bio::Genex::UserCanonicalLink;
...
;
  $TABLE2PKEY = {
          'CanonicalSequenceFeature' => 'usf_fk',
          'UserSequenceFeature' => 'csf_fk'
        }
;

  $COLUMN2NAME  = {
          'csf_fk' => 'Canonical Sequence Feature',
          'usf_fk' => 'User Sequence Feature'
        }
;
  $NAME2COLUMN  = {
          'Canonical Sequence Feature' => 'csf_fk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['usf_fk', 'csf_fk', 'usf_obj', 'csf_obj']);

sub table_name {return 'UserCanonicalLink';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::UserCanonicalLink::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::UserCanonicalLink::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::UserCanonicalLink::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::UserCanonicalLink::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::UserCanonicalLink::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::UserCanonicalLink::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::UserCanonicalLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::UserCanonicalLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::UserCanonicalLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::UserCanonicalLink::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::UserCanonicalLink::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::UserCanonicalLink::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['usf_fk', 'csf_fk'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::UserCanonicalLink::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::UserCanonicalLink::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "UserCanonicalLink: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::UserCanonicalLink - Methods for processing data from the GeneX DB
linking table: UserCanonicalLink

=head1 SYNOPSIS

  use Bio::Genex::UserCanonicalLink;

  # instantiating a linking table instance
  my $UserCanonicalLink = Bio::Genex::UserCanonicalLink->new(id=>47,pkey_link=>'usf_fk');
  # or 
  my $UserCanonicalLink = Bio::Genex::UserCanonicalLink->new(id=>47,pkey_link=>'csf_fk');

  # retrieve data from the DB for all columns
  $UserCanonicalLink->fetch();

  # creating an instance, without pre-fetching all columns
  my $UserCanonicalLink = new Bio::Genex::UserCanonicalLink(id=>47,
                                              'pkey_link'=>'usf_fk');
  ...
  my @objects = Bio::Genex::UserCanonicalLink->get_objects('pkey_link'=>'usf_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::UserCanonicalLink->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $UserCanonicalLink->id();

  # retrieving other DB column attributes
  my $usf_fk_val = $UserCanonicalLink->usf_fk();
  $UserCanonicalLink->usf_fk($value);

  my $csf_fk_val = $UserCanonicalLink->csf_fk();
  $UserCanonicalLink->csf_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::UserCanonicalLink is
UserCanonicalLink).


B<NOTE>: Class Bio::Genex::UserCanonicalLink is linking table class. This means
...
value from the database. In Bio::Genex::UserCanonicalLink, those two columns are
'usf_fk' and 'usf_fk'. When an instance of
Bio::Genex::UserCanonicalLink is instantiated, either via C<new()> or
...
Most applications will first create an instance of Bio::Genex::UserCanonicalLink
...
Bio::Genex::UserCanonicalLink can access: I<raw> foreign key attributes,
...
Class Bio::Genex::UserCanonicalLink defines the following utility variables for assisting
programmers to access the UserCanonicalLink table.

=over 4

=item $Bio::Genex::UserCanonicalLink::LIMIT
...
=item $Bio::Genex::UserCanonicalLink::USE_CACHE
...
For example:
  ...
  my $UserCanonicalLink = Bio::Genex::UserCanonicalLink->new(id=>47);
  my $val = $UserCanonicalLink->usf_fk();
...
instance of the class via the Bio::Genex::UserCanonicalLink->methodname() syntax.
...
DB. For class Bio::Genex::UserCanonicalLink, 'C<pkey_link>' can be either 'C<usf_fk>' or 'C<csf_fk>'.
...
Bio::Genex::UserCanonicalLink if a method in class C<UserSequenceFeature> invokes its
foreign key accessor function, the column 'csf_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::UserCanonicalLink it returns 1, since it I<is>
...
Bio::Genex::UserCanonicalLink it returns 'UserCanonicalLink';
...
for the UserCanonicalLink table.

=item column_names()

This method returns an array ref which holds the names of all the
columns in table UserCanonicalLink.
...
Bio::Genex::UserCanonicalLink simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the UserCanonicalLink table and returns a single
...
a warning from C<Bio::Genex::UserCanonicalLink::initialize()>. Objects will be
...
instance of class Bio::Genex::UserCanonicalLink.
...
instance. Class Bio::Genex::UserCanonicalLink is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<usf_fk>' and 'C<csf_fk>'). When an instance of
class Bio::Genex::UserCanonicalLink is created using either C<new()> or
...
Methods for the csf_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
on Mon Jan 15 11:06:38 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=UserCanonicalLink
