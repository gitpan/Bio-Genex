...
# Bio::Genex::UserSec
#
# created on Mon Jan 15 11:06:32 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=UserSec --support=GroupLink
#
# cvs id: $Id: UserSec.pm,v 1.16.2.1 2001/01/15 18:52:02 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'con_fk' => 'Contact',
          'login' => 'Login',
          'us_pk' => 'Accession Number'
        }
;
  $NAME2COLUMN  = {
          'Contact' => 'con_fk',
          'Accession Number' => 'us_pk',
          'Password' => 'password',
          'Login' => 'login'
        }
;
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['us_pk', 'con_fk', 'login', 'password', 'con_obj', 'grouplink_obj', 'grouplink_fk']);

sub table_name {return 'UserSec';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'UserSec',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'us_pk=' . $dbh->quote($self->us_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::UserSec::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::UserSec::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::UserSec::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::UserSec::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::UserSec::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::UserSec::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::UserSec::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::UserSec::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['us_pk', 'con_fk', 'login', 'password'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::UserSec::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::UserSec::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "UserSec: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
on Mon Jan 15 11:06:32 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=UserSec --support=GroupLink
