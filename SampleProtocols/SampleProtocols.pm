...
#
# created on Mon Jan 15 11:06:36 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=SampleProtocols
#
# cvs id: $Id: SampleProtocols.pm,v 1.9.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['smp_fk', 'prt_fk', 'comment', 'smp_obj', 'prt_obj']);

sub table_name {return 'SampleProtocols';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::SampleProtocols::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::SampleProtocols::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::SampleProtocols::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::SampleProtocols::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::SampleProtocols::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::SampleProtocols::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::SampleProtocols::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::SampleProtocols::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::SampleProtocols::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::SampleProtocols::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::SampleProtocols::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::SampleProtocols::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['smp_fk', 'prt_fk', 'comment'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::SampleProtocols::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::SampleProtocols::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "SampleProtocols: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::SampleProtocols - Methods for processing data from the GeneX DB
linking table: SampleProtocols
...
=head1 SYNOPSIS
  ...
  my $SampleProtocols = Bio::Genex::SampleProtocols->new(id=>47,pkey_link=>'smp_fk');
  # or 
  my $SampleProtocols = Bio::Genex::SampleProtocols->new(id=>47,pkey_link=>'prt_fk');

  # retrieve data from the DB for all columns
  $SampleProtocols->fetch();

  # creating an instance, without pre-fetching all columns
  my $SampleProtocols = new Bio::Genex::SampleProtocols(id=>47,
                                              'pkey_link'=>'smp_fk');
  ...
  my @objects = Bio::Genex::SampleProtocols->get_objects('pkey_link'=>'smp_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::SampleProtocols->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $SampleProtocols->id();

  # retrieving other DB column attributes
  my $smp_fk_val = $SampleProtocols->smp_fk();
  $SampleProtocols->smp_fk($value);

  my $prt_fk_val = $SampleProtocols->prt_fk();
  $SampleProtocols->prt_fk($value);

  my $comment_val = $SampleProtocols->comment();
  $SampleProtocols->comment($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::SampleProtocols is
SampleProtocols).


B<NOTE>: Class Bio::Genex::SampleProtocols is linking table class. This means
...
value from the database. In Bio::Genex::SampleProtocols, those two columns are
'smp_fk' and 'smp_fk'. When an instance of
Bio::Genex::SampleProtocols is instantiated, either via C<new()> or
...
Bio::Genex::SampleProtocols can access: I<raw> foreign key attributes,
...
Class Bio::Genex::SampleProtocols defines the following utility variables for assisting
programmers to access the SampleProtocols table.

=over 4

=item $Bio::Genex::SampleProtocols::LIMIT
...
=item $Bio::Genex::SampleProtocols::USE_CACHE
...
For example:
  ...
  my $SampleProtocols = Bio::Genex::SampleProtocols->new(id=>47);
  my $val = $SampleProtocols->smp_fk();
...
instance of the class via the Bio::Genex::SampleProtocols->methodname() syntax.
...
DB. For class Bio::Genex::SampleProtocols, 'C<pkey_link>' can be either 'C<smp_fk>' or 'C<prt_fk>'.
...
Bio::Genex::SampleProtocols if a method in class C<Sample> invokes its
foreign key accessor function, the column 'prt_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::SampleProtocols it returns 1, since it I<is>
...
Bio::Genex::SampleProtocols it returns 'SampleProtocols';
...
=item get_objects({pkey_link=>'smp_fk'}, @id_list)
...
Bio::Genex::SampleProtocols simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the SampleProtocols table and returns a single
...
a warning from C<Bio::Genex::SampleProtocols::initialize()>. Objects will be
...
instance of class Bio::Genex::SampleProtocols.
...
instance. Class Bio::Genex::SampleProtocols is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<smp_fk>' and 'C<prt_fk>'). When an instance of
class Bio::Genex::SampleProtocols is created using either C<new()> or
...
Methods for the smp_fk attribute. This attribute is the first
...
Methods for the prt_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
Methods for the comment attribute.
...
on Mon Jan 15 11:06:36 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=SampleProtocols
