...
#
# created on Mon Jan 15 11:06:28 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=ArrayLayout --support=AL_Spots
#
# cvs id: $Id: ArrayLayout.pm,v 1.13.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::ArrayLayout::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::ArrayLayout::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::ArrayLayout::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::ArrayLayout::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::ArrayLayout::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
=head1 SYNOPSIS
  ...
  my @objects = Bio::Genex::ArrayLayout->get_objects('ALL');
...
of the same name (I<i.e.> the corresponding table for Bio::Genex::ArrayLayout is
ArrayLayout).
...
Bio::Genex::ArrayLayout can access: I<raw> foreign key attributes,
...
Class Bio::Genex::ArrayLayout defines the following utility variables for assisting
programmers to access the ArrayLayout table.

=over 4

=item $Bio::Genex::ArrayLayout::LIMIT
...
=item $Bio::Genex::ArrayLayout::USE_CACHE
...
instance of the class via the Bio::Genex::ArrayLayout->methodname() syntax.
...
('al_pk' for module Bio::Genex::ArrayLayout).
...
a warning from C<Bio::Genex::ArrayLayout::initialize()>. Objects will be
...
instance of class Bio::Genex::ArrayLayout.
...
=item @id_list = al_spots_fk()

=item @obj_list = al_spots_obj()

This is an attribute of type ONE_TO_MANY_LT and refers to class
L<Bio::Genex::AL_Spots>. The raw accessor method, C<al_spots_fk()> returns a list of
foreign key ids. The OO accessor method, C<al_spots_obj()> returns a
...
C<al_pk()>method). The C<id()> method can be useful in writing
...
=item $value = technology_type();

=item technology_type($value);
...
=item $value = default_spot_conc();

=item default_spot_conc($value);

Methods for the default_spot_conc attribute.


=item $value = default_spot_conc_units();

=item default_spot_conc_units($value);

Methods for the default_spot_conc_units attribute.


=item $value = default_spot_conc_error();

=item default_spot_conc_error($value);
...
on Mon Jan 15 11:06:28 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=ArrayLayout --support=AL_Spots
