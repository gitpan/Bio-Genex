...
# Bio::Genex::AM_Spots
#
# created on Mon Jan 15 11:06:32 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=AM_Spots --support=SpotLink
#
# cvs id: $Id: AM_Spots.pm,v 1.9.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'ams_pk' => 'Accession Number'
        }
;
  $NAME2COLUMN  = {
          'Accession Number' => 'ams_pk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['ams_pk', 'am_fk', 'usf_fk', 'spot_value', 'usf_obj', 'spotlink_obj', 'spotlink_fk', 'am_obj']);

sub table_name {return 'AM_Spots';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'AM_Spots',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'ams_pk=' . $dbh->quote($self->ams_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::AM_Spots::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::AM_Spots::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::AM_Spots::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::AM_Spots::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::AM_Spots::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::AM_Spots::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub get_matrix {
  ...
  if (scalar @args == 1 && ref($args[0]) ne 'ARRAY') {
   ...
   $args{COLUMNS} = ['ams_pk', 'usf_fk', 'spot_value'];
    ...
    $args{WHERE} = $WHERE;
    $args{COLUMNS} = ['ams_pk', 'am_fk', 'usf_fk', 'spot_value'];
  }
  ...
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::AM_Spots::get_matrix: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::AM_Spots::get_matrix: $DBI::errstr";
  ...
  while (my @vals = $sth->fetchrow_array()) {
    ...
    die "Bio::Genex::AM_Spots::get_matrix: $DBI::errstr" if $sth->err;
    push(@objects,\@vals);
  }
  $sth->finish();
  return \@objects;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::AM_Spots::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::AM_Spots::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['ams_pk', 'am_fk', 'usf_fk', 'spot_value'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::AM_Spots::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::AM_Spots::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "AM_Spots: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
on Mon Jan 15 11:06:32 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=AM_Spots --support=SpotLink
