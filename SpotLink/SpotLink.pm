...
# Bio::Genex::SpotLink
#
# created on Mon Jan 15 11:06:35 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=SpotLink
#
# cvs id: $Id: SpotLink.pm,v 1.9.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'als_fk' => 'Array Layout Spot'
        }
;
  $NAME2COLUMN  = {
          'Array Measurement Spot' => 'ams_fk',
          'Array Layout Spot' => 'als_fk'
        }
;
  $FKEY_OBJ2RAW = {
          'ams_obj' => 'ams_fk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['ams_fk', 'als_fk', 'ams_obj', 'als_obj']);

sub table_name {return 'SpotLink';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::SpotLink::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::SpotLink::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::SpotLink::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::SpotLink::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::SpotLink::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::SpotLink::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::SpotLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::SpotLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::SpotLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::SpotLink::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::SpotLink::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::SpotLink::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['ams_fk', 'als_fk'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::SpotLink::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::SpotLink::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "SpotLink: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::SpotLink - Methods for processing data from the GeneX DB
linking table: SpotLink
...
=head1 SYNOPSIS
  ...
  my $SpotLink = Bio::Genex::SpotLink->new(id=>47,pkey_link=>'ams_fk');
  # or 
  my $SpotLink = Bio::Genex::SpotLink->new(id=>47,pkey_link=>'als_fk');

  # retrieve data from the DB for all columns
  $SpotLink->fetch();

  # creating an instance, without pre-fetching all columns
  my $SpotLink = new Bio::Genex::SpotLink(id=>47,
                                              'pkey_link'=>'ams_fk');
  ...
  my @objects = Bio::Genex::SpotLink->get_objects('pkey_link'=>'ams_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::SpotLink->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $SpotLink->id();

  # retrieving other DB column attributes
  my $ams_fk_val = $SpotLink->ams_fk();
  $SpotLink->ams_fk($value);

  my $als_fk_val = $SpotLink->als_fk();
  $SpotLink->als_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::SpotLink is
SpotLink).


B<NOTE>: Class Bio::Genex::SpotLink is linking table class. This means
...
value from the database. In Bio::Genex::SpotLink, those two columns are
'ams_fk' and 'ams_fk'. When an instance of
Bio::Genex::SpotLink is instantiated, either via C<new()> or
...
Bio::Genex::SpotLink can access: I<raw> foreign key attributes,
...
Class Bio::Genex::SpotLink defines the following utility variables for assisting
programmers to access the SpotLink table.

=over 4

=item $Bio::Genex::SpotLink::LIMIT
...
=item $Bio::Genex::SpotLink::USE_CACHE
...
For example:
  ...
  my $SpotLink = Bio::Genex::SpotLink->new(id=>47);
  my $val = $SpotLink->ams_fk();
...
instance of the class via the Bio::Genex::SpotLink->methodname() syntax.
...
DB. For class Bio::Genex::SpotLink, 'C<pkey_link>' can be either 'C<ams_fk>' or 'C<als_fk>'.
...
Bio::Genex::SpotLink if a method in class C<AM_Spots> invokes its
foreign key accessor function, the column 'als_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::SpotLink it returns 1, since it I<is>
...
Bio::Genex::SpotLink it returns 'SpotLink';
...
=item get_objects({pkey_link=>'ams_fk'}, @id_list)
...
Bio::Genex::SpotLink simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the SpotLink table and returns a single
...
a warning from C<Bio::Genex::SpotLink::initialize()>. Objects will be
...
instance of class Bio::Genex::SpotLink.
...
instance. Class Bio::Genex::SpotLink is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<ams_fk>' and 'C<als_fk>'). When an instance of
class Bio::Genex::SpotLink is created using either C<new()> or
...
Methods for the ams_fk attribute. This attribute is the first
...
Methods for the als_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
on Mon Jan 15 11:06:35 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=SpotLink
