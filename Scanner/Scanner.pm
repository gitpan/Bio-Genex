...
# Bio::Genex::Scanner
#
# created on Mon Jan 15 11:06:33 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=Scanner
#
# cvs id: $Id: Scanner.pm,v 1.13.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'scn_pk' => 'Accession Number'
        }
;
  $NAME2COLUMN  = {
          'Contact' => 'con_fk',
          'Accession Number' => 'scn_pk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['scn_pk', 'model_serial_num', 'model_description', 'con_fk', 'con_obj']);

sub table_name {return 'Scanner';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'Scanner',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'scn_pk=' . $dbh->quote($self->scn_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::Scanner::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::Scanner::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::Scanner::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::Scanner::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::Scanner::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::Scanner::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::Scanner::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::Scanner::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['scn_pk', 'model_serial_num', 'model_description', 'con_fk'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::Scanner::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::Scanner::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "Scanner: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::Scanner - Methods for processing data from the GeneX DB
 table: Scanner
...
=head1 SYNOPSIS
  ...
  my $Scanner = Bio::Genex::Scanner->new(id=>47);

  # retrieve data from the DB for all columns
  $Scanner->fetch();

  # creating an instance, without pre-fetching all columns
  my $Scanner = new Bio::Genex::Scanner(id=>47);

  # creating an instance with pre-fetched data
  my $Scanner = new Bio::Genex::Scanner(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::Scanner->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::Scanner->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $Scanner->id();

  # or specifically
  my $scn_pk_val = $Scanner->scn_pk();

  # retreving other DB column attributes
  my $model_serial_num_val = $Scanner->model_serial_num();
  $Scanner->model_serial_num($value);

  my $model_description_val = $Scanner->model_description();
  $Scanner->model_description($value);

  my $con_fk_val = $Scanner->con_fk();
  $Scanner->con_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::Scanner is
Scanner).
...
Bio::Genex::Scanner can access: I<raw> foreign key attributes,
...
Class Bio::Genex::Scanner defines the following utility variables for assisting
programmers to access the Scanner table.

=over 4

=item $Bio::Genex::Scanner::LIMIT
...
=item $Bio::Genex::Scanner::USE_CACHE
...
instance of the class via the Bio::Genex::Scanner->methodname() syntax.
...
linking table class. For Bio::Genex::Scanner it returns 0, since it is
...
table classes, and for Bio::Genex::Scanner it returns the value 'scn_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::Scanner it returns 'Scanner';
...
C<insert_db()>.
  ...
  my Scanner = Bio::Genex::Scanner->new();
  Scanner->model_serial_num('some_value');
  Scanner->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my Scanner = Bio::Genex::Scanner->new(id=>43);
  Scanner->model_serial_num('some_value');
  Scanner->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('scn_pk' for module Bio::Genex::Scanner).
...
This method is used to retrieve multiple instances of class Bio::Genex::Scanner
...
list as a primary key for the Scanner table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::Scanner::initialize()>. Objects will be
...
instance of class Bio::Genex::Scanner.
...
(and for class Bio::Genex::Scanner it is synonomous with the
C<scn_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item scn_pk()

This is the primary key attribute for Bio::Genex::Scanner. It has no setter method. 
...
on Mon Jan 15 11:06:33 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=Scanner
