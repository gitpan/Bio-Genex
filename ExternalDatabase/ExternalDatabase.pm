...
#
# created on Mon Jan 15 11:06:34 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=ExternalDatabase
#
# cvs id: $Id: ExternalDatabase.pm,v 1.10.2.1 2001/01/15 18:52:01 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          'main_url' => 'URL',
          'parameterized_url' => 'Is  URL Parameterized',
          'name' => 'Accession Number'
        }
;
  $NAME2COLUMN  = {
          'Is  URL Parameterized' => 'parameterized_url',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['name', 'main_url', 'parameterized_url']);

sub table_name {return 'ExternalDatabase';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'ExternalDatabase',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'name=' . $dbh->quote($self->name());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::ExternalDatabase::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::ExternalDatabase::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::ExternalDatabase::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::ExternalDatabase::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::ExternalDatabase::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::ExternalDatabase::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::ExternalDatabase::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::ExternalDatabase::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['name', 'main_url', 'parameterized_url'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::ExternalDatabase::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::ExternalDatabase::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "ExternalDatabase: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::ExternalDatabase - Methods for processing data from the GeneX DB
 table: ExternalDatabase
...
=head1 SYNOPSIS
  ...
  $ExternalDatabase->fetch();

  # creating an instance, without pre-fetching all columns
  my $ExternalDatabase = new Bio::Genex::ExternalDatabase(id=>47);

  # creating an instance with pre-fetched data
  my $ExternalDatabase = new Bio::Genex::ExternalDatabase(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::ExternalDatabase->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::ExternalDatabase->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $ExternalDatabase->id();

  # or specifically
  my $name_val = $ExternalDatabase->name();

  # retreving other DB column attributes
  my $main_url_val = $ExternalDatabase->main_url();
  $ExternalDatabase->main_url($value);

  my $parameterized_url_val = $ExternalDatabase->parameterized_url();
  $ExternalDatabase->parameterized_url($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::ExternalDatabase is
ExternalDatabase).
...
Bio::Genex::ExternalDatabase can access: I<raw> foreign key attributes,
...
Class Bio::Genex::ExternalDatabase defines the following utility variables for assisting
programmers to access the ExternalDatabase table.

=over 4

=item $Bio::Genex::ExternalDatabase::LIMIT
...
=item $Bio::Genex::ExternalDatabase::USE_CACHE
...
instance of the class via the Bio::Genex::ExternalDatabase->methodname() syntax.
...
C<insert_db()>.
  ...
  ExternalDatabase->insert_db($dbh);
...
C<update_db()>.
  ...
  ExternalDatabase->main_url('some_value');
  ExternalDatabase->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('name' for module Bio::Genex::ExternalDatabase).
...
instance of class Bio::Genex::ExternalDatabase.
...
C<name()>method). The C<id()> method can be useful in writing
...
=item $value = parameterized_url();

=item parameterized_url($value);
...
on Mon Jan 15 11:06:34 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=ExternalDatabase
