...
#
# created on Mon Jan 15 11:06:36 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=CSF_ExternalDBLink
#
# cvs id: $Id: CSF_ExternalDBLink.pm,v 1.9.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
package Bio::Genex::CSF_ExternalDBLink;
...
;
  $TABLE2PKEY = {
          'CanonicalSequenceFeature' => 'db_name',
          'ExternalDatabase' => 'csf_fk'
        }
;
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id', 'pkey_link'], lookup=>['csf_fk', 'db_name', 'external_identifier', 'csf_obj', 'db_name_obj']);

sub table_name {return 'CSF_ExternalDBLink';} # probably unnecessary
...
sub pkey_name {
  ...
  if (not ref($self)) {
    ...
    my $ref_table = shift;
    die "Bio::Genex::CSF_ExternalDBLink::pkey_name: Must specify a table argument" 
      unless defined $ref_table;
    # just in case we're given a class name
    $ref_table =~ s/Bio::Genex:://;
    $name = $TABLE2PKEY->{$ref_table};
    die "Bio::Genex::CSF_ExternalDBLink::pkey_name: table $ref_table not in TABLE2PKEY" 
      unless defined $name;
  } else {
    # instance method requires pkey_link
    $name = $self->get_attribute('pkey_link');
    die "Bio::Genex::CSF_ExternalDBLink::pkey_name: must set the 'pkey_link' attribute for linking tables classes" unless defined $name;
  }
  return $name;
}
#
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::CSF_ExternalDBLink::get_objects: Must define both 'column' and 'value'" 
    ...
    die "Bio::Genex::CSF_ExternalDBLink::get_objects: must set 'pkey_link' for linking class" 
      unless exists $hashref->{pkey_link};
    $pkey_name = $hashref->{pkey_link};
  }
  my @ids = @_;
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::CSF_ExternalDBLink::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::CSF_ExternalDBLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::CSF_ExternalDBLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::CSF_ExternalDBLink::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    #
    die "Bio::Genex::CSF_ExternalDBLink::initialize: Must set 'pkey_link' when calling new() with an id" 
    ...
    die "Bio::Genex::CSF_ExternalDBLink::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::CSF_ExternalDBLink::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['csf_fk', 'db_name', 'external_identifier'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::CSF_ExternalDBLink::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::CSF_ExternalDBLink::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "CSF_ExternalDBLink: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
on Mon Jan 15 11:06:36 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=CSF_ExternalDBLink
