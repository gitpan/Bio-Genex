...
#
# created on Mon Jan 15 11:06:30 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=ExperimentFactors --support=TL_FactorValues --support=AM_FactorValues
#
# cvs id: $Id: ExperimentFactors.pm,v 1.10.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
...
;
 $FKEYS = {
          'tl_factorvalues_fk' => bless( {
                                           'table_name' => 'TL_FactorValues',
                                           'fkey_type' => 'ONE_TO_MANY_LINK',
                                           'fkey_name' => 'tl_factorvalues_fk',
                                           'pkey_name' => 'ef_fk'
                                         }, 'Bio::Genex::Fkey' ),
          ...
          'tl_factorvalues_obj' => bless( {
                                            'fkey_type' => 'ONE_TO_MANY_LINK_OO',
                                            'table_name' => 'TL_FactorValues',
                                            'fkey_name' => 'tl_factorvalues_obj',
                                            'pkey_name' => 'ef_fk'
                                          }, 'Bio::Genex::Fkey' ),
...
;
  ...
  $COLUMN2NAME  = {
          ...
          'es_fk' => 'Experiment Set',
          'ef_pk' => 'Accession Number',
...
;
  $NAME2COLUMN  = {
          'Accession Number' => 'ef_pk',
          'Major Category' => 'major_category',
          'Minor Category' => 'minor_category',
          'Factor Units' => 'factor_units',
          'Factor Name' => 'factor_name',
          'Experiment Set' => 'es_fk'
        }
;
  $FKEY_OBJ2RAW = {
          'tl_factorvalues_obj' => 'tl_factorvalues_fk',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['ef_pk', 'es_fk', 'factor_name', 'factor_units', 'major_category', 'minor_category', 'am_factorvalues_obj', 'am_factorvalues_fk', 'tl_factorvalues_obj', 'tl_factorvalues_fk', 'es_obj']);

sub table_name {return 'ExperimentFactors';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'ExperimentFactors',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'ef_pk=' . $dbh->quote($self->ef_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::ExperimentFactors::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::ExperimentFactors::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::ExperimentFactors::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::ExperimentFactors::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::ExperimentFactors::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::ExperimentFactors::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::ExperimentFactors::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::ExperimentFactors::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['ef_pk', 'es_fk', 'factor_name', 'factor_units', 'major_category', 'minor_category'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::ExperimentFactors::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::ExperimentFactors::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "ExperimentFactors: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::ExperimentFactors - Methods for processing data from the GeneX DB
 table: ExperimentFactors
...
=head1 SYNOPSIS
  ...
  my @objects = Bio::Genex::ExperimentFactors->get_objects('ALL');
...
For example:
  ...
  my $ExperimentFactors = Bio::Genex::ExperimentFactors->new(id=>47);
  my $val = $ExperimentFactors->ef_pk();
...
instance of the class via the Bio::Genex::ExperimentFactors->methodname() syntax.
...
linking table class. For Bio::Genex::ExperimentFactors it returns 0, since it is
...
table classes, and for Bio::Genex::ExperimentFactors it returns the value 'ef_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::ExperimentFactors it returns 'ExperimentFactors';
...
for the ExperimentFactors table.

=item column_names()

This method returns an array ref which holds the names of all the
columns in table ExperimentFactors.
...
C<insert_db()>.
  ...
  my ExperimentFactors = Bio::Genex::ExperimentFactors->new();
  ExperimentFactors->es_fk('some_value');
  ExperimentFactors->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my ExperimentFactors = Bio::Genex::ExperimentFactors->new(id=>43);
  ExperimentFactors->es_fk('some_value');
  ExperimentFactors->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('ef_pk' for module Bio::Genex::ExperimentFactors).
...
This method is used to retrieve multiple instances of class Bio::Genex::ExperimentFactors
...
list as a primary key for the ExperimentFactors table and returns a single
...
instance of class Bio::Genex::ExperimentFactors.
...
(and for class Bio::Genex::ExperimentFactors it is synonomous with the
C<ef_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item ef_pk()

This is the primary key attribute for Bio::Genex::ExperimentFactors. It has no setter method. 
...
=item $value = factor_name();

=item factor_name($value);

Methods for the factor_name attribute.


=item $value = factor_units();

=item factor_units($value);

Methods for the factor_units attribute.


=item $value = major_category();

=item major_category($value);

Methods for the major_category attribute.


=item $value = minor_category();

=item minor_category($value);

Methods for the minor_category attribute.
...
on Mon Jan 15 11:06:30 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=ExperimentFactors --support=TL_FactorValues --support=AM_FactorValues
