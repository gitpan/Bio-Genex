...
#
# created on Mon Jan 15 11:06:31 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=CanonicalSequenceFeature --support=RelatedCSF --support=CSF_ExternalDBLink
#
# cvs id: $Id: CanonicalSequenceFeature.pm,v 1.10.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
package Bio::Genex::CanonicalSequenceFeature;
...
;
 $FKEYS = {
          'csf_externaldblink_fk' => bless( {
                                              'table_name' => 'CSF_ExternalDBLink',
                                              'fkey_type' => 'ONE_TO_MANY_LINK',
                                              'fkey_name' => 'csf_externaldblink_fk',
                                              'pkey_name' => 'csf_fk'
                                            }, 'Bio::Genex::Fkey' ),
          'csf_externaldblink_obj' => bless( {
                                               'fkey_type' => 'ONE_TO_MANY_LINK_OO',
                                               'table_name' => 'CSF_ExternalDBLink',
                                               'fkey_name' => 'csf_externaldblink_obj',
                                               'pkey_name' => 'csf_fk'
                                             }, 'Bio::Genex::Fkey' ),
          'spc_obj' => bless( {
                                'table_name' => 'Species',
          ...
          'relatedcsf_fk' => bless( {
                                      'table_name' => 'RelatedCSF',
                                      'fkey_type' => 'ONE_TO_MANY_LINK',
                                      'fkey_name' => 'relatedcsf_fk',
                                      'pkey_name' => 'csf2_csf_fk'
                                    }, 'Bio::Genex::Fkey' ),
          'relatedcsf_obj' => bless( {
                                       'fkey_type' => 'ONE_TO_MANY_LINK_OO',
                                       'table_name' => 'RelatedCSF',
                                       'fkey_name' => 'relatedcsf_obj',
                                       'pkey_name' => 'csf2_csf_fk'
                                     }, 'Bio::Genex::Fkey' )
        }
;
  ...
  $COLUMN2NAME  = {
          ...
          'csf_pk' => 'Accession Number',
          'short_description' => 'Short Description',
          'spc_fk' => 'Species',
          'csf_type' => 'Canonical Sequence Feature Type',
          'csf_name' => 'Canonical Sequence Feature Name'
        }
;
  $NAME2COLUMN  = {
          'Species' => 'spc_fk',
          'Accession Number' => 'csf_pk',
          'Short Description' => 'short_description',
          'Canonical Sequence Feature Type' => 'csf_type',
          'Chromosome Location' => 'chromosome',
          'Canonical Sequence Feature Name' => 'csf_name'
        }
;
  $FKEY_OBJ2RAW = {
          'csf_externaldblink_obj' => 'csf_externaldblink_fk',
          'spc_obj' => 'spc_fk',
          'relatedcsf_obj' => 'relatedcsf_fk'
        }
;
}


attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['csf_pk', 'spc_fk', 'csf_name', 'csf_type', 'chromosome', 'short_description', 'relatedcsf_obj', 'relatedcsf_fk', 'relatedcsf_obj', 'relatedcsf_fk', 'csf_externaldblink_obj', 'csf_externaldblink_fk', 'spc_obj']);

sub table_name {return 'CanonicalSequenceFeature';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'CanonicalSequenceFeature',\%values);
  $dbh->do($sql);
  
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::CanonicalSequenceFeature::insert_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  my $pkey = fetch_last_id($dbh,'CanonicalSequenceFeature');
  $self->id($pkey);
  $self->csf_pk($pkey);
  return $pkey;
}
...
sub update_db {
  my ($self,$dbh) = @_;
  assert_dbh($dbh);
  die "Bio::Genex::CanonicalSequenceFeature::update_db: object not in DB"
    unless defined $self->id() && defined $self->csf_pk();
  ...
  # execute the UPDATE
  my $WHERE = 'csf_pk=' . $dbh->quote($self->csf_pk());
  my $sql = create_update_sql($dbh,
			      TABLE=>'CanonicalSequenceFeature',
			      SET=>\%values,
			      WHERE=>$WHERE);
  $dbh->do($sql);
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::CanonicalSequenceFeature::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::CanonicalSequenceFeature::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::CanonicalSequenceFeature::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::CanonicalSequenceFeature::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::CanonicalSequenceFeature::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::CanonicalSequenceFeature::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::CanonicalSequenceFeature::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::CanonicalSequenceFeature::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['csf_pk', 'spc_fk', 'csf_name', 'csf_type', 'chromosome', 'short_description'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::CanonicalSequenceFeature::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::CanonicalSequenceFeature::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "CanonicalSequenceFeature: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::CanonicalSequenceFeature - Methods for processing data from the GeneX DB
 table: CanonicalSequenceFeature

=head1 SYNOPSIS

  use Bio::Genex::CanonicalSequenceFeature;

  # instantiating an instance
  my $CanonicalSequenceFeature = Bio::Genex::CanonicalSequenceFeature->new(id=>47);

  # retrieve data from the DB for all columns
  $CanonicalSequenceFeature->fetch();

  # creating an instance, without pre-fetching all columns
  my $CanonicalSequenceFeature = new Bio::Genex::CanonicalSequenceFeature(id=>47);

  # creating an instance with pre-fetched data
  my $CanonicalSequenceFeature = new Bio::Genex::CanonicalSequenceFeature(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::CanonicalSequenceFeature->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::CanonicalSequenceFeature->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $CanonicalSequenceFeature->id();

  # or specifically
  my $csf_pk_val = $CanonicalSequenceFeature->csf_pk();

  # retreving other DB column attributes
  my $spc_fk_val = $CanonicalSequenceFeature->spc_fk();
  $CanonicalSequenceFeature->spc_fk($value);

  my $csf_name_val = $CanonicalSequenceFeature->csf_name();
  $CanonicalSequenceFeature->csf_name($value);

  my $csf_type_val = $CanonicalSequenceFeature->csf_type();
  $CanonicalSequenceFeature->csf_type($value);

  my $chromosome_val = $CanonicalSequenceFeature->chromosome();
  $CanonicalSequenceFeature->chromosome($value);

  my $short_description_val = $CanonicalSequenceFeature->short_description();
  $CanonicalSequenceFeature->short_description($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::CanonicalSequenceFeature is
CanonicalSequenceFeature).


Most applications will first create an instance of Bio::Genex::CanonicalSequenceFeature
...
Bio::Genex::CanonicalSequenceFeature can access: I<raw> foreign key attributes,
...
Class Bio::Genex::CanonicalSequenceFeature defines the following utility variables for assisting
programmers to access the CanonicalSequenceFeature table.

=over 4

=item $Bio::Genex::CanonicalSequenceFeature::LIMIT
...
=item $Bio::Genex::CanonicalSequenceFeature::USE_CACHE
...
For example:
  ...
  my $CanonicalSequenceFeature = Bio::Genex::CanonicalSequenceFeature->new(id=>47);
  my $val = $CanonicalSequenceFeature->csf_pk();
...
instance of the class via the Bio::Genex::CanonicalSequenceFeature->methodname() syntax.
...
linking table class. For Bio::Genex::CanonicalSequenceFeature it returns 0, since it is
...
table classes, and for Bio::Genex::CanonicalSequenceFeature it returns the value 'csf_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::CanonicalSequenceFeature it returns 'CanonicalSequenceFeature';
...
for the CanonicalSequenceFeature table.

=item column_names()

This method returns an array ref which holds the names of all the
columns in table CanonicalSequenceFeature.
...
C<insert_db()>.
  ...
  my CanonicalSequenceFeature = Bio::Genex::CanonicalSequenceFeature->new();
  CanonicalSequenceFeature->spc_fk('some_value');
  CanonicalSequenceFeature->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my CanonicalSequenceFeature = Bio::Genex::CanonicalSequenceFeature->new(id=>43);
  CanonicalSequenceFeature->spc_fk('some_value');
  CanonicalSequenceFeature->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('csf_pk' for module Bio::Genex::CanonicalSequenceFeature).
...
This method is used to retrieve multiple instances of class Bio::Genex::CanonicalSequenceFeature
...
list as a primary key for the CanonicalSequenceFeature table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::CanonicalSequenceFeature::initialize()>. Objects will be
...
instance of class Bio::Genex::CanonicalSequenceFeature.
...
=item @id_list = relatedcsf_fk()

=item @obj_list = relatedcsf_obj()

This is an attribute of type ONE_TO_MANY_LINK and refers to class
L<Bio::Genex::RelatedCSF>. The raw accessor method, C<relatedcsf_fk()> returns a list of
foreign key ids. The OO accessor method, C<relatedcsf_obj()> returns a
list of objects of class Bio::Genex::RelatedCSF.
...
=item @id_list = csf_externaldblink_fk()

=item @obj_list = csf_externaldblink_obj()

This is an attribute of type ONE_TO_MANY_LINK and refers to class
L<Bio::Genex::CSF_ExternalDBLink>. The raw accessor method, C<csf_externaldblink_fk()> returns a list of
foreign key ids. The OO accessor method, C<csf_externaldblink_obj()> returns a
list of objects of class Bio::Genex::CSF_ExternalDBLink.
...
(and for class Bio::Genex::CanonicalSequenceFeature it is synonomous with the
C<csf_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item csf_pk()

This is the primary key attribute for Bio::Genex::CanonicalSequenceFeature. It has no setter method. 
...
Methods for the csf_name attribute.
...
Methods for the csf_type attribute.
...
on Mon Jan 15 11:06:31 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=CanonicalSequenceFeature --support=RelatedCSF --support=CSF_ExternalDBLink
