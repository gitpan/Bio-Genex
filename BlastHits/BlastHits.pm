...
# Bio::Genex::BlastHits
#
# created on Mon Jan 15 11:06:34 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=BlastHits
#
# cvs id: $Id: BlastHits.pm,v 1.10.2.1 2001/01/15 18:52:00 jes Exp $ 
#
##############################
...
;
  ...
  $COLUMN2NAME  = {
          'bh_pk' => 'Accession Number',
          'expect_value' => 'Expectation Value',
          'usf_fk' => 'User Sequence Feature',
          'match_accession' => 'Match Accession'
        }
;
  $NAME2COLUMN  = {
          'Expectation Value' => 'expect_value',
          'Accession Number' => 'bh_pk',
          'Match Accession' => 'match_accession',
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['bh_pk', 'usf_fk', 'match_accession', 'expect_value', 'usf_obj']);

sub table_name {return 'BlastHits';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'BlastHits',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # execute the UPDATE
  my $WHERE = 'bh_pk=' . $dbh->quote($self->bh_pk());
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::BlastHits::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::BlastHits::get_objects: Must define both 'column' and 'value'" 
  ...
  if (scalar @ids == 0 && ! $has_args) {
    croak("Bio::Genex::BlastHits::get_objects called with no ID's
");
  } elsif (scalar @ids == 0 ||
    ...
    my $sth = $dbh->prepare($sql) 
      or die "Bio::Genex::BlastHits::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    $sth->execute() 
      or die "Bio::Genex::BlastHits::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
    ...
    die "Bio::Genex::BlastHits::get_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
      if $sth->err;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::BlastHits::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::BlastHits::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['bh_pk', 'usf_fk', 'match_accession', 'expect_value'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::BlastHits::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::BlastHits::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "BlastHits: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
on Mon Jan 15 11:06:34 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=BlastHits
